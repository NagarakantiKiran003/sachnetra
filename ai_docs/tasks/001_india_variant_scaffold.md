...
## Session Notes — Issues & Lessons for Future Agents

### Issue 1 — Rules file pointed to wrong file for hostname detection
**Rule**: `india-variant.md` says to add hostname detection in `src/App.ts`.
**Reality**: Hostname detection happens in `src/config/variant.ts` (an IIFE that runs at module load). `App.ts` never does variant detection — it only consumes the already-resolved `SITE_VARIANT` constant.
**What we did**: Added `if (h.includes('sachnetra')) return 'india';` to `variant.ts`, not `App.ts`. Correct behaviour.
**Future agents**: If any rule mentions `App.ts` for variant detection, ignore it. Always edit `src/config/variant.ts`.

---

### Issue 2 — `panels.ts` uses chained ternaries, not a switch
**What a future agent might assume**: A `switch (SITE_VARIANT)` block or an object map.
**Reality**: Three separate `export const X = SITE_VARIANT === 'happy' ? ... : FULL_X;` chained ternaries, one for `DEFAULT_PANELS`, one for `DEFAULT_MAP_LAYERS`, one for `MOBILE_DEFAULT_MAP_LAYERS`. All three must be extended independently.
**Future agents**: Always extend all 3 ternaries. Missing one means the map layers or mobile layers won't resolve correctly for the india variant.

---

### Issue 3 — `india` missing from localhost localStorage whitelist
**Symptom**: Even after env var was set correctly, switching variant via `localStorage.setItem('worldmonitor-variant', 'india')` in DevTools did nothing.
**Cause**: The localhost branch in `variant.ts` had an explicit allowlist (`tech | full | finance | happy | commodity`) that did not include `india`. The stored value was ignored and fell through to `buildVariant`.
**Fix**: Added `|| stored === 'india'` to the whitelist in Phase 2 (Step 2.2).

---

### Issue 4 — `.env.local` was UTF-16 LE, Vite requires UTF-8
**Symptom**: Console showed `current="full"` even after localStorage was cleared, even though `VITE_VARIANT=india` was in `.env.local`.
**Cause**: The file had been created/edited on Windows in UTF-16 LE encoding. Vite could not parse it, so `import.meta.env.VITE_VARIANT` came back `undefined`, falling through to `|| 'full'`.
**Diagnosis signal**: `view_file` on `.env.local` returned error `unsupported mime type text/plain; charset=utf-16le`.
**Fix**: Deleted and recreated `.env.local` using `Set-Content -Encoding UTF8` in PowerShell.
**Future agents**: If `VITE_VARIANT` env var appears correct but the variant still resolves to `full`, suspect encoding. Always create/write `.env.local` with UTF-8. VS Code does this correctly; Notepad on Windows may use UTF-16.

---

### Issue 5 — Dev server runs on port 3000, not Vite's default 5173
**Symptom**: `localhost:5173` returned nothing; browser showed blank or connection refused.
**Cause**: `vite.config.ts` sets `port: 3000` explicitly.
**Future agents**: Always open `http://localhost:3000` for this project, not 5173.
